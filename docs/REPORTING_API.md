...
7. **`export` sí funciona a nivel de servidor (verificado en vivo el 2026-08-09/10) pero tiene el mismo bloqueo de CORS del punto 6** — reproducido con `OPTIONS` idéntico (`204` sin ningún header `Access-Control-Allow-*`). O sea: aunque el frontend mande la petición perfecta, el navegador la va a tumbar hasta que el equipo de Reporting habilite CORS.
8. **`export`/`trends`: el parámetro `metrics` va como query param REPETIDO, no como lista separada por comas.** `?metrics=heartRate,steps` se interpreta como un solo código desconocido y responde `422 {"errors":{"Metrics[0]":["Unknown metric code 'heartRate,steps'."]}}`. Lo correcto es `?metrics=heartRate&metrics=steps` (o `metrics[0]=`/`metrics[1]=`, también válido). El código de este frontend (`lib/api/reporting.ts`) ya se corrigió para mandarlo así.
9. **Catálogo de códigos de métrica válidos para `metrics`, confirmado en vivo probando uno por uno** (probamos ~35 nombres candidatos; estos 8 son los únicos que no devuelven `422 Unknown metric code`):
   `heartRate`, `steps`, `weight`, `height`, `systolicBp`, `diastolicBp`, `spo2`, `hrv`.
   Notablemente **no existen** códigos para calorías, horas sedentarias/activas, IMC ni sueño pese a probar muchas variantes razonables (`calories`, `caloriesBurned`, `sedentaryHours`, `bmi`, `sleep`, etc. — todos `422`) — el catálogo de este servicio parece limitarse a signos vitales tipo wearable (MedicalDataService), no a las métricas de sedentarismo/gamificación que sí expone Dashboard Service.
